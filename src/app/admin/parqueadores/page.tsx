    ...
    <tbody className="bg-white divide-y divide-gray-200">
      {data.map((p) => (
        <tr key={p.id} className="hover:bg-gray-50 transition-colors duration-150">
          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-700">{p.id}</td>
          ...
          <td className="px-4 py-2 whitespace-nowrap text-right text-sm font-medium space-x-2">
            <Link href={`/admin/parqueadores/${p.id}`}>
              <Button size="sm" variant="outline">Editar</Button>
            </Link>
            <Button size="sm" variant="destructive" onClick={() => alert('Aquí iría eliminar')}>Eliminar</Button>
          </td>
        </tr>
      ))}
    </tbody>
