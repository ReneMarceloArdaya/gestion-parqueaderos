...
import { getOperadorById, updateOperador } from '@/lib/Supabase/operadores'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'

export default function UpdateOperadorPage() {
  ...
  useEffect(() => {
    async function load() {
      const op = await getOperadorById(supabase, id)
      setNombre(op.nombre ?? '')
      setEmail(op.contacto_email ?? '')
      setTelefono(op.contacto_telefono ?? '')
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    try {
      await updateOperador(supabase, id, {
        nombre,
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50">
      <Card className="w-full max-w-lg shadow-xl rounded-2xl">
        <CardHeader className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-1">Actualiza los datos del operador</p>
        </CardHeader>
        <CardContent>
