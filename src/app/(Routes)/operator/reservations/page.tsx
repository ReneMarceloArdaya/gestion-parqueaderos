...
import { CalendarDays, Clock, Car, MapPin, User, Phone, Mail, CheckCircle, XCircle, Filter, RefreshCw } from 'lucide-react'
...
export default function ReservationsPage() {
  ...
  const [filtroFecha, setFiltroFecha] = useState<'todas' | 'hoy' | 'proximas24h' | 'urgentes'>('todas')
  ...
  useEffect(() => {
    const initializePage = async () => {
      try {
        ...
        const { data: reservasData, error: reservasError } = await (supabase as any)
          .from('reservas')
          .select('*')
          .eq('estado', 'activa')
  ...
  const contactarUsuario = (reserva: ReservaDetallada) => {
    ...
    const subject = `Reserva #${reserva.codigo_reserva || reserva.id} - ${reserva.parqueadero_nombre}`
...
Equipo de ${reserva.parqueadero_nombre}`
    ...
    const mailtoLink = `mailto:${reserva.usuario_email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`
      ...
      <div className="mb-8">
        <div className="flex justify-between items-start">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={recargarDatos} disabled={loading} variant="outline">
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            ...
            >
              Próximas 24h ({reservas.filter(r => {
                const inicio = new Date(r.hora_inicio)
                const ahora = new Date()
                const en24h = new Date(ahora.getTime() + 24 * 60 * 60 * 1000)
                return inicio >= ahora && inicio <= en24h
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold">
              Mostrando {reservasFiltered.length} reserva{reservasFiltered.length !== 1 ? 's' : ''}
            </h2>
          ...
          <div className="grid gap-6">
            {reservasFiltered.map((reserva) => {
              ...
              return (
              <Card key={reserva.id} className={`${isUrgent ? 'border-red-200 bg-red-50' : ''}`}>
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="flex items-center gap-2">
                        <span>Reserva #{reserva.codigo_reserva || reserva.id}</span>
                        {isUrgent && <Badge variant="destructive">Urgente</Badge>}
                      </CardTitle>
                      <CardDescription>
                        Creada el {formatDateTime(reserva.creado_at).date}
                      </CardDescription>
                      <div className={`text-sm font-medium mt-1 ${timeRemaining.color}`}>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1 text-sm">
                        <p className="font-medium">{reserva.usuario_nombre || 'Usuario sin nombre'}</p>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1 text-sm">
                        <p className="font-medium">{reserva.parqueadero_nombre}</p>
                        <p className="text-gray-600">{reserva.parqueadero_direccion}</p>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2 text-sm">
                        <div>
                          ...
                          <p className="text-gray-600">{inicioFormatted.date}</p>
                          <p className="text-gray-600">{inicioFormatted.time}</p>
                        </div>
                        <div>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span>Duración: {formatDuration(reserva.duracion_minutos)}</span>
                        </div>
                  ...
                  <div className="flex gap-3 mt-6 pt-4 border-t">
                    ...
                    >
                      ...
                      {actionLoading === `confirm-${reserva.id}` ? 'Confirmando...' : 'Confirmar'}
                    </Button>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Esta acción cancelará la reserva #{reserva.codigo_reserva || reserva.id} de{' '}
